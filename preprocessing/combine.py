...
base_path = "cleaning/" 
# final_combined_path = "combined_sensor_data-all-2.csv"
final_combined_path = "combined_sensor_data-all-4.csv" # with static features

# C02 has 

# volunteers = [ "C01", "C03", "C04","C05", "C06", "C07", "C08", "C10", "C11"] 
activities = {
...
# # Function to resample and align sensor data to a common frequency (1 second)
...
#         # Convert to datetime and drop rows where conversion fails
#         sensor_data['timestamp'] = pd.to_datetime(sensor_data['timestamp'], errors='coerce')
#         if sensor_data['timestamp'].isnull().any():
...
#         # Set index as 'timestamp' and resample
#         print(f"before resampling {sensor_data.columns}")
#         sensor_data = sensor_data.set_index('timestamp').resample(frequency).ffill()
#         # Reset the index and rename it to avoid conflicts
#         sensor_data = sensor_data.reset_index().rename(columns={'timestamp': 'resampled_timestamp'})
#         print(f"after resampling {sensor_data.columns}")
#         # Rename 'resampled_timestamp' back to 'timestamp'
#         sensor_data = sensor_data.rename(columns={'resampled_timestamp': 'timestamp'})
#     return sensor_data
...
#         if sensor_data['timestamp'].isnull().any():
#             print("Warning: Some timestamps could not be converted to datetime.")
#             sensor_data = sensor_data.dropna(subset=['timestamp'])  # Drop rows with NaT timestamps
#         sensor_data = sensor_data.drop_duplicates(subset=['timestamp'])
#         sensor_data = sensor_data.set_index('timestamp')
#         sensor_data = sensor_data.resample(frequency).ffill()
        
#         print("Columns before resetting index:", sensor_data.columns)
...
def adjusted_speed_threshold(height, age, fitness_level, weight, base_threshold=1.5, reference_height=1.7, reference_age=30, decline_rate=0.01, ideal_bmi=22):
    ...
    # Final speed threshold after weight adjustment
...
# Convert the static data to a DataFrame
    ...
    for category, activity_list in activities.items():
        for activity in activity_list:
            ...
            if 'physiologicalState' in uwb_data_cleaned.columns:
                ...
                # uwb_data = pd.concat([uwb_data.drop('physiologicalState', axis=1), physiological_state_encoded], axis=1)
...
combined_data.to_csv(final_combined_path, index=False)
    ...
    #             # round time to nearest seocnd
    #             uwb_data['timestamp'] = uwb_data['timestamp'].dt.round('S')  
    #             netatmo_data['timestamp'] = netatmo_data['timestamp'].dt.round('S')  
    #             motion_data['timestamp'] = motion_data['timestamp'].dt.round('S') 

    #             # merge data on the rounded timestamp 
    #             merged_data = pd.merge(uwb_data, motion_data, on="timestamp", how="inner")
    #             merged_data = pd.merge(netatmo_data, merged_data, on="timestamp", how="inner")

    #             merged_data['class'] = activity_labels[activity]

    #             combined_data = pd.concat([combined_data, merged_data], ignore_index=True)

    # combined_data.to_csv("comb_data.csv", index=False)
